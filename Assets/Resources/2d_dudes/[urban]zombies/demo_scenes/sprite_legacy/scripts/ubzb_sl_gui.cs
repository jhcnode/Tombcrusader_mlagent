...
public class ubzb_sl_gui : MonoBehaviour {
	...
	void next_body_skin (int ch_no)
	{
		...
		int temp_inarm_weapon_no = skinchanger[ch_no].in_arm_weapon_skin.Length;
		int temp_inarm_shield_no = skinchanger[ch_no].in_arm_shield_skin.Length;
		
		if(next_body_no >= skinchanger[ch_no].body_skin.Length)
		{
			next_body_no = 0;
		}
		
		if( now_inarm_no[ch_no] < skinchanger[ch_no].in_arm_weapon_skin.Length )
		{
			temp_inarm_weapon_no = now_inarm_no[ch_no];
		}
		else
		{
			temp_inarm_shield_no = now_inarm_no[ch_no] - skinchanger[ch_no].in_arm_weapon_skin.Length;
		}
	...
	void next_outarm_skin (int ch_no)
	{
		...
		if(next_outarm_no > skinchanger[ch_no].out_arm_weapon_skin.Length)
		{
			next_outarm_no = 0;
		}
	...
	void next_inarm_skin (int ch_no)
	{
		...
		if(next_inarm_no > ( skinchanger[ch_no].in_arm_weapon_skin.Length + skinchanger[ch_no].in_arm_shield_skin.Length ) )
		{
			next_inarm_no = 0;
		}
	...
	void next_animation (int ch_no)
	{
		...
		if(next_ani_no >= ( anichanger[ch_no].ani_list.Length + anichanger[ch_no].ani_list_facehurt.Length ) )
		{
			next_ani_no = 0;
		}
		...
		{
			temp_ani_no = next_ani_no - anichanger[ch_no].ani_list.Length;
			temp_ani_name = anichanger[ch_no].ani_list_facehurt[temp_ani_no];
		}
		
		anichanger [ch_no].ani_player (temp_ani_name, "loop");
	...
	public ubzb_sl_skinchanger[] skinchanger;
	public ubzb_sl_anicontroller[] anichanger;
